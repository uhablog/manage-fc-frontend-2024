...
export async function GET(
  ...
  const assists = Array.isArray(json) ? json : Array.isArray(json?.data) ? json.data : [];
  ...
  const enrichedAssists = assists.map((item: { auth0_user_id?: string }) => ({
    ...item,
    emblem_url:
  ...
  return Response.json(enrichedAssists);
};
