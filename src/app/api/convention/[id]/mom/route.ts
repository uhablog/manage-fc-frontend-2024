...
export async function GET(
  ...
  const momRank = Array.isArray(json) ? json : Array.isArray(json?.data) ? json.data : [];

  let emblemUrls: Record<string, string | null> = {};
  try {
    const token = ensureBlobToken();
    const userIds = momRank.map((item: { auth0_user_id?: string }) => item.auth0_user_id ?? null);
    emblemUrls = await getLatestBlobUrls(userIds, token);
  } catch (error) {
    console.error("Failed to resolve MOM rank emblems", error);
  }

  const enrichedMomRank = momRank.map((item: { auth0_user_id?: string }) => ({
    ...item,
    emblem_url:
  ...
  return Response.json(enrichedMomRank);
};
